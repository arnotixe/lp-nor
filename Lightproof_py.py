...
from lightproof_impl_%s import locales
from lightproof_impl_%s import pkg
import lightproof_handler_%s
...
# check settings
def option(lang, opt):
    return lightproof_handler_%s.get_option(lang.Language + "_" + lang.Country, opt)
...
class Lightproof( unohelper.Base, XProofreader, XServiceInfo, XServiceName, XServiceDisplayName, XSupportedLocales):
    ...
    def __init__( self, ctx, *args ):
        ...
        lightproof_handler_%s.load(currentContext)
...
g_ImplementationHelper.addImplementation( lightproof_handler_%s.LightproofOptionsEventHandler, \\
